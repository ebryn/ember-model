(function() {
  ...
  Ember.Model = Ember.Object.extend(Ember.Evented, {
    ...
    */
    ...
    load: function(id, hash) {
      ...
      for (let [relationshipKey, relationshipMeta] of this.constructor.relationships) {
        if (relationshipMeta.options.embedded && ! relationshipMeta.options.polymorphic) {
          ...
          // PATCH(bburley): Use transformed data key for relationships.
          var dataKey = this.dataKey(relationshipKey);
    ...
    revert: function() {
      ...
      // PATCH(b marie)
      // dirty attributes were getting added with nested models sometime after calling reloadHasManys
      // this started happening in Ember 2.1
      this.getWithDefault('_dirtyAttributes', Ember.A([])).clear();
    },
      ...
      for (i = 0; i < this._hasManyArrays.length; i++) {
        ...
        if (!reverting) {
          for (j = 0; j < array.get('length'); j++) {
            // PATCH(johnweathers): In some cases, a hasMany array has had an object removed
            // from it, but its cached length has not been updated yet and in such cases
            // we might have an invalid index value for 'j' here. So we need to add an initial
            // check to this conditional to make sure that we have an object at location j before
            // trying to operate on it
            if (array.objectAt(j) && array.objectAt(j).get('isNew') && !array.objectAt(j).get('isDeleted')) {
              // PATCH(mpa): Sometimes when reloading data received from the server for a freshly created
              // object, a stale cached reference to the original unsaved object is still in the hasManyContent
              // and yet our new fully loaded copy of the object has a difference reference and so if we
              // do not first remove the stale copy before adding the loaded copy, we will have two copies
              // in memory and see a client-side only duplicate of this object
              var ref, existing;
              hasManyContent.addObject(array.objectAt(j)._reference);
              ref = array.objectAt(j)._reference;
              if (ref.record && ref.record.id) {
                existing = hasManyContent.findBy('id', ref.record.id);
                if (existing) {
                  hasManyContent.removeObject(existing);
                }
              }
              hasManyContent.addObject(ref);
            }
          }
        }
      ...
      if (content) {
        ...
        if (embedded) {
          ...
          mapFunction = function(attrs) {
            // PATCH(mattwright): The primary key may be a computed property here.
            // This seems to be happening for the ItemModifierListAggregated model
            // at least. Using `Ember.get` ensures we're get the value and not the
            // `computed` function. The better fix would be to prevent this from
            // happening in the first place, but after hours of trying I can't
            // figure out where this is happening.
            //
            // TODO(mattwright): Attempt to rip this out once we've moved past
            // Ember 1.13 and/or refactored or (ideally) removed the hella complex
            // COGS/multiunit/aggregate item code.
            reference = type._getOrCreateReferenceForId(Ember.get(attrs, primaryKey));
  ...
  Ember.Model.reopenClass({
    ...
    load: function(hashes, container) {
      ...
      for (var i = 0, l = hashes.length; i < l; i++) {
        ...
        var record = this.getCachedReferenceRecord(primaryKey, container);
  ...
  Ember.attr = function(type, options) {
    return Ember.computed("_data", {
      ...
      set: function(key, value){
        ...
        // PATCH(mattwright): Checking `dataValue !== serialize(value, type)` will
        // always evaluate as dirty for attributes that are objects (Money attrs
        // for example), so we add special handling here.
        newValue = serialize(value, type);
        if (typeof newValue === "object") {
          isDirty = JSON.stringify(dataValue) !== JSON.stringify(newValue);
        } else {
          isDirty = dataValue !== newValue;
        }
  ...
  Ember.Model.Store = Ember.Object.extend({
    ...
    // define an attribute called `container`. I (mattwright) created an issue
    // with more details here: https://github.com/ebryn/ember-model/issues/457
    // container: null,
